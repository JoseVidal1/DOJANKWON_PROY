...
const Sidebar = () => {
  ...
  return (
    <>
      <aside className={`sticky top-0 h-[100dvh] min-h-screen transition-all duration-300 ${expanded ? "w-70" : "w-16"}`}>
        <nav className="h-full flex flex-col border-0 shadow-xl" style={{ backgroundColor: "var(--primary-background-color)", borderColor: "var(--primary-border-color)" }}>
          <div className={`p-4 pb-2 flex items-center transition-all duration-300 ${expanded ? "justify-between" : "justify-center"}`}>
            ...
            {expanded && (
             <div className="flex flex-col text-center ml-2">
               <h1 className="text-xl font-bold" style={{ color: "var(--primary-text-color)" }}>Alvaro Vidal M.</h1>
               <h2 className="text-sm font-josefin" style={{ color: "var(--secundary-text-color)" }}>Administrador</h2>
               </div>
            )}
            <button onClick={() => setExpanded((curr) => !curr)} className="p-1.5 rounded-lg" style={{ backgroundColor: "var(--secundary-background-color)" }}>
    ...
    <div className={`border-t flex items-center p-3 transition-all duration-300 ${expanded ? "gap-3" : "justify-center"}`} style={{ borderTopColor: "var(--custom-border-color)" }}>
      ...
      <div className={`overflow-hidden transition-all duration-300 ${expanded ? "w-52 ml-3" : "w-0"}`}>
        <div className="flex items-center justify-between leading-4">
          <div>
            ...
            <span className="text-xs" style={{ color: "var(--secundary-text-color)" }}>AvidalMartinez@gmail.com</span>
          </div>
