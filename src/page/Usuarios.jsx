...
const Usuarios = () => {
  ...
  return (
    ...
    >
      ...
      {/* Lineas decorativas verticales debajo */}
      <hr className="absolute left-[-70px] right-[-70px] border-t border-[color:var(--secundary-dark-color)]" />
      <p className="font-josefin ml-2">
        {" "}
        Gestiona de forma{" "}
        <span style={{ color: "var(--accent-dark-color)" }}>segura</span> a los
        usuarios del sistema.
        <br />
        controla sus permisos de{" "}
        <span style={{ color: "var(--accent-dark-color)" }}>acceso </span>de
        forma eficiente
      </p>
      <hr className=" left-[-70px] right-[-70px] border-t border-[color:var(--secundary-dark-color)]" />
      <h1
        className=" mt-10 text-sm font-bold ml-2  font-josefin"
        style={{ color: "var(--accent-dark-color)" }}
      >
        DATOS DE USUARIO
      </h1>
      ...
      <form className="w-full flex flex-wrap gap-x-6 gap-y-4 mt-10 px-4">
        ...
        <div className="w-full md:flex-1 flex flex-col min-w-0">
          <label
            htmlFor="usuario"
            className="text-sm font-josefin font-semibold text-[var(--text-dark-color)] uppercase"
          >
            Usuario
          </label>
          <input
            ...
            placeholder="Usuario"
            required
            className="bg-transparent border-b-2 border-[var(--accent-dark-color)] text-white p-1 focus:outline-none focus:border-b-[3px] transition"
          />
        </div>
        ...
        <div className="w-full flex justify-center mt-6">
          <button
            className="group relative inline-flex h-11 items-center justify-center overflow-hidden rounded-md px-6 font-medium transition hover:scale-105 duration-300"
            style={{
              backgroundColor: "var(--terceary-dark-color)",
              tercearyColor: "var(--terceary-dark-color)",
      ...
      {/* Presentacion tabla */}
      <h1
        className="text-sm ml-2 mt-20  font-josefin"
        style={{ color: "var(--accent-dark-color)" }}
      >
        taekwondo
      </h1>
      <hr className="left-[-70px] right-[-70px] border-t border-[color:var(--secundary-dark-color)]" />
      <h1
        className="text-2xl sm:text-2xl ml-2 md:text-4xl lg:text-5xl font-josefin mb-1 font-medium text-left"
        style={{ color: "var(--text-dark-color)" }}
      >
        TABLA DE USUARIOS
      </h1>
      ...
      <div className="p-5 mt-10">
        ...
        <div className="mb-4 w-full flex flex-col md:flex-row md:items-center gap-2">
          <div className="relative w-full md:w-1/3">
            <input
              ...
              className="w-full pl-10 pr-3 py-2 border rounded shadow-sm focus:outline-none focus:ring-2 text-sm"
              style={{
                backgroundColor: "var(--sidebar-dark-hover)",
                borderColor: "var(--terceary-dark-color)",
                color: "var(--text-dark-color)",
                outlineColor: "var(--terceary-dark-color)",
              }}
            />
            <button
              type="button"
              className="absolute inset-y-0 left-0 px-3 flex items-center text-[#F44E1C] hover:text-[#f44e1c66]"
            >
              <svg
                className="w-4 h-4"
                fill="none"
              ...
              {usuarios.map((u, idx) => (
                ...
                >
                  {[
                    ...
                    <td key={field} className="p-2">
                      <input
                        ...
                        value={u[field]}
                        onChange={(e) =>
                          InputCambios(u.id, field, e.target.value)
                        }
                  ...
                  <td className="p-3 flex justify-center space-x-2">
                    <button
                      onClick={() => GuardarU(u)}
                      className="p-1 text-sm bg-gray-700 rounded hover:bg-gray-900"
                    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
          {usuarios.map((u) => (
            <div
              key={u.id}
              className="space-y-3 p-4 rounded-lg shadow"
              style={{
                backgroundColor: "var(--secundary-dark-color)",
                color: "var(--text-dark-color)",
              }}
